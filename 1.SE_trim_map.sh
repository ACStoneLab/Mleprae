...
#This script performs the following on SE sequencing data
...
eval "$(conda shell.bash hook)"  #necessary for calling conda activate env?
conda activate working

set -ex
#set -uex #prevents the next commands from being executed after a command has failed
#also keeps track of which command is being executed and prints that to the command line

#reference only needs to be indexed once
bwa index $ref
samtools faidx $ref
...
echo -e "Sample \t Total Reads \t Analysis-ready reads (trimmed) \t Proportion of reads kept after trimming \t Mapped reads \t Proportion of reads mapped (Mapped reads / Analysis-ready reads) \t Q37 mapped reads \t Unique Q37 mapped reads \t Average length of mapped reads \t Mean coverage \t Std dev of mean coverage \t % reference covered >= 1X \t % reference covered >= 5X \t Cluster factor (Mapped reads/ Unique Q37 mapped reads) \t % endogenous DNA (Unique Q37 mapped reads / Total reads)" > $outputdir/modern_SE_leprae_pipeline_summary.txt
...
for a in $path/Sample*
 do
  ...
  fastqc $path/"${ID}"/*.fastq.gz -o $outputdir/$ID/1.raw_fastqc 
  ...
  AdapterRemoval --file1 $path/"${ID}"/*.fastq.gz  --trimqualities --trimns --minquality 20 --minlength 30 --settings $outputdir/"${ID}"/2.adapter_removal/"${NAME}"_settingz.txt --output1 $outputdir/"${ID}"/2.adapter_removal/"${NAME}"_trimmed.fastq --discarded $outputdir/"${ID}"/2.adapter_removal/"${NAME}"_discarded.fastq
...
#--minlength 30 #sets a min length for kept fragments
  ...
  echo "Running fastqc on trimmed and merged reads "${NAME}"!" 
  
  mkdir $outputdir/$ID/3.post_AR_fastqc
  fastqc $outputdir/"${ID}"/2.adapter_removal/"${NAME}"_trimmed.fastq -o $outputdir/"${ID}"/3.post_AR_fastqc


#adapter removal stats
  total_reads=`grep "Total number of reads" $outputdir/"${ID}"/2.adapter_removal/"${NAME}"_settingz.txt | awk '{print $NF}'`
  analysis_ready=`grep "Number of retained reads" $outputdir/"${ID}"/2.adapter_removal/"${NAME}"_settingz.txt | awk '{print $NF}'`
  pc_kept=`echo "scale=4;$analysis_ready/$total_reads" | bc`
...
## Here we are using only the trimmed and merged reads as input. Modify the following if you need to use unmerged R1 and R2 as well.
  bwa aln -l 1000 -n 0.1 -t 8 $ref $outputdir/"${ID}"/2.adapter_removal/"${NAME}"_trimmed.fastq  > $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed.sai
  
  bwa samse $ref $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed.sai $outputdir/"${ID}"/2.adapter_removal/"${NAME}"_trimmed.fastq > $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed.sam
...
## (-q 37), and alignments containing the 4 flag (0x4 segment unmapped)
  ...
  samtools view -bh -F4 $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed.bam > $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed_mapped.bam

  samtools view -bh -q 37 $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed_mapped.bam > $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed_mapped_q37.bam
  
## Sort alignments by leftmost coordinates

  samtools sort -o $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed_mapped_q37_sorted.bam $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed_mapped_q37.bam
  
## Print alignment statistics after removing low quality reads

  echo "Mapped reads" >> $outputdir/"${ID}"/4.bwa/"${NAME}"_MappingReport.txt
  samtools view -c $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed_mapped.bam >> $outputdir/"${ID}"/4.bwa/"${NAME}"_MappingReport.txt
...
## if multiple read pairs have identical external coordinates, only retain the pair with highest mapping quality
  ...
  samtools markdup -r $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed_mapped_q37_sorted.bam $outputdir/"${ID}"/4.bwa/"${NAME}"_trimmed_mapped_q37_sorted_rmdup.bam
...
## Calculate statistics
  ...
  echo -e "${NAME} \t $total_reads \t $analysis_ready \t $pc_kept \t $mapped \t $pc_mapped \t $Q37 \t $unique \t $length \t $mean_cov \t $sd_cov \t $cov1 \t $cov5 \t $cf \t $endo" >> $outputdir/modern_SE_leprae_pipeline_summary.txt
